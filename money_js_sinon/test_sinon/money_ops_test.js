...
QUnit.test("test sub with result < 0", function(assert)
	{
		var m1=new money(2,"EUR");
		sinon.stub(m1,"getValue").returns(1);
		sinon.stub(m1,"getCurrency").returns("EUR");

		var m2=new money(6,"EUR");
		sinon.stub(m2,"getValue").returns(2);
		sinon.stub(m2,"getCurrency").returns("EUR");
		assert.throws(function(assert) {var m3=MoneyOps.sub(m1, m2);}, SubValueNegException, "negative value");
	}
)
...
;QUnit.test("test simple sub", function(assert)
	{
		...
		sinon.stub(m1,"getValue").returns(3);
		sinon.stub(m1,"getCurrency").returns("EUR");
