...
QUnit.test("test currency have more then 3 character", function(assert)
	{
		assert.throws(function(assert) {var m=new money(5,"EEEE");}, DeviseCaracSupException, "currency have more then 3 character");
		assert.throws(function(assert) {var m=new money(50.5,"DIRHAME");}, DeviseCaracSupException, "currency have more then 3 character");
	}
);
QUnit.test("test negative value", function(assert)
	{
		assert.throws(function(assert) {var m=new money(-1,"EUR");}, ValueNegativeException, " negative value");
		assert.throws(function(assert) {var m=new money(-11.1,"EUR");}, ValueNegativeException, " negative value");
	}
);
